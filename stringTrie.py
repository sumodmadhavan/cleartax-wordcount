...
newNode
	...
	def writeToDisk(self, rootPath):
		...
		while len(openSet) > 0:
			...
			if currNode.count != 0:
				endFile = open(currPath + '/end',
'a')
				for i in range(currNode.count):
					endFile.write('1')
				endFile.close()

			for character, childNode in
currNode.children.iteritems():
				openSet.append((childNode, currPath
+ '/' + character))
